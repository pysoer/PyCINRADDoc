# 标准格式基数据
-这里支持的格式有：标准格式雷达、标准格式相控阵雷达基数据    
-**PyCINRAD独有** 两种数据一个接口`cinrad.io.StandardData`或`cinrad.io.read_auto`
## 数据读取
...
```
```python
vel0_xy = cinrad.calc.polar_to_xy(vel0, resolution= (1000, 1000))
vel0_xy
```
```md
